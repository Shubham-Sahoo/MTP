...
class ASRDataset(BaseDataset):
    ...
    def read_entries(self):
        ...
        self.entries = [line.split("\t", 5) for line in self.entries]
        # print(self.entries)
    ...
    def generator(self):
        for path1, _, path2, _, _, indices in self.entries:
            audio1 = load_and_convert_to_wav(path1).numpy()
            audio2 = load_and_convert_to_wav(path2).numpy()
            yield bytes(path1, "utf-8"), audio1, bytes(path2, "utf-8"), audio2, bytes(indices, "utf-8")

    def preprocess(self, path1: tf.Tensor, audio1: tf.Tensor, path2: tf.Tensor, audio2: tf.Tensor, indices: tf.Tensor):
        with tf.device("/CPU:0"):
            def fn(_path1: bytes, _audio1: bytes, _path2: bytes, _audio2: bytes, _indices: bytes):
                signal1 = read_raw_audio(_audio1, sample_rate=self.speech_featurizer.sample_rate)
                signal2 = read_raw_audio(_audio2, sample_rate=self.speech_featurizer.sample_rate)

                signal1 = self.augmentations.signal_augment(signal1)
                signal2 = self.augmentations.signal_augment(signal2)

                features1 = self.speech_featurizer.extract(signal1.numpy())
                features1 = self.augmentations.feature_augment(features1)
                features1 = tf.convert_to_tensor(features1, tf.float32)

                features2 = self.speech_featurizer.extract(signal2.numpy())
                features2 = self.augmentations.feature_augment(features2)
                features2 = tf.convert_to_tensor(features2, tf.float32)

                features = tf.concat([features1,features2],0)
                ...
                return _path1, features, input_length, label, label_length, prediction, prediction_length
    ...
    def tf_preprocess(self, path1: tf.Tensor, audio1: tf.Tensor, path2: tf.Tensor, audio2: tf.Tensor, indices: tf.Tensor):
        with tf.device("/CPU:0"):
            signal1 = tf_read_raw_audio(audio1, self.speech_featurizer.sample_rate)
            signal2 = tf_read_raw_audio(audio2, self.speech_featurizer.sample_rate)

            signal1 = self.augmentations.signal_augment(signal1)
            signal2 = self.augmentations.signal_augment(signal2)

            features1 = self.speech_featurizer.tf_extract(signal1)
            features1 = self.augmentations.feature_augment(features1)

            features2 = self.speech_featurizer.tf_extract(signal2)
            features2 = self.augmentations.feature_augment(features2)
            ...
            return path1, features, input_length, label, label_length, prediction, prediction_length

    def parse(self, path1: tf.Tensor, audio1: tf.Tensor, path2: tf.Tensor, audio2: tf.Tensor, indices: tf.Tensor):
        """
        Returns:
            path, features, input_lengths, labels, label_lengths, pred_inp
        """
        data = self.tf_preprocess(path1, audio1, path2, audio2, indices) if self.use_tf else self.preprocess(path1, audio1, path2, audio2, indices)
    ...
    def create(self, batch_size: int):
        ...
        dataset = tf.data.Dataset.from_generator(
            ...
            output_shapes=(tf.TensorShape([]), tf.TensorShape([]), tf.TensorShape([]), tf.TensorShape([]), tf.TensorShape([]))
        )
...
class ASRSliceDataset(ASRDataset):
    ...
    def load(record: tf.Tensor):
        def fn(path1: bytes, path2: bytes): 
            audio1 = load_and_convert_to_wav(path1.decode("utf-8")).numpy()
            audio2 = load_and_convert_to_wav(path2.decode("utf-8")).numpy()
            return audio1, audio2
        audio1, audio2 = tf.numpy_function(fn, inp=[record[0], record[2]], Tout=[tf.string, tf.string])
        return record[0], audio1, record[2], audio2, record[5]
